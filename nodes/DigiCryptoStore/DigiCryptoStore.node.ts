...
export class DigiCryptoStore implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'DigiCryptoStore by CHECKHC',
		name: 'digiCryptoStore',
		icon: 'file:photocertif.png',
		group: ['transform'],
		version: 1,
		subtitle: '={{$parameter["operation"]}} - Document',
		description: 'DigiCryptoStore - Document certification on Solana blockchain with permanent Arweave storage. Developed by CHECKHC. Learn more: https://www.checkhc.net',
		defaults: {
			name: 'DigiCryptoStore by CHECKHC',
		},
		inputs: ['main'],
		outputs: ['main'],
		credentials: [
		...
		properties: [
			// Resource Type
			{
				...
				type: 'hidden',
				default: 'docs',
				description: 'Type of content to certify - Fixed to Document (media/docs)',
			},
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				if (operation !== 'listNfts') {
					const credentials = await this.getCredentials('digiCryptoStoreApi', i);
					baseUrl = (credentials.digiCryptoStoreUrl as string).replace(/\/$/, '');
